...
def main() -> int:
    ...
    run_parser.add_argument(
        "-o", "--output", type=Path, default=Path("runs"), help="Output directory"
    )
    run_parser.add_argument(
        "-n", "--n-trajectories", type=int, default=None, help="Override number of trajectories"
    )
    run_parser.add_argument(
        "-m",
        "--mode",
        choices=["counterfactual", "open_ended", "population"],
        default=None,
        help="Override simulation mode",
    )
    run_parser.add_argument(
        "-c", "--concurrency", type=int, default=2, help="Max parallel trajectories"
    )
    ...
    run_parser.add_argument(
        "--dry-run", action="store_true", help="Validate and summarize scenario without running"
    )
    run_parser.add_argument(
        "--provider",
    ...
    report_parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to run output directory (default: latest in runs/)",
    )
    report_parser.add_argument(
        "--format",
        dest="output_format",
    ...
    viz_parser.add_argument(
        "--fields", nargs="+", default=None, help="State fields to plot over time"
    )
    viz_parser.add_argument(
        "--populations", nargs="+", default=None, help="Population names for score plots"
    )
    viz_parser.add_argument(
        "--scores", nargs="+", default=None, help="Score fields for population plots"
    )
    viz_parser.add_argument(
        "--types",
        nargs="+",
    ...
    dash_parser.add_argument(
        "--scores", nargs="+", default=None, help="Score fields for populations"
    )
    dash_parser.add_argument(
        "--static", action="store_true", help="Generate static HTML instead of launching server"
    )
    dash_parser.add_argument(
        "-o",
        "--output",
        ...
        default=None,
        dest="dash_output",
        help="Output path for static HTML",
    )
    ...
    init_parser = subparsers.add_parser(
        "init-scenario", help="Generate a template scenario YAML file"
    )
    ...
    explore_parser.add_argument(
        "--fields", nargs="+", default=None, help="State fields to visualize (default: auto-detect)"
    )
...
def cmd_run(args) -> int:
    ...
    if scenario.resampling:
        ...
        trajectories = asyncio.run(run_batch(scenario, output_dir, args.concurrency, args.timeout))
...
def cmd_compare(args) -> int:
    ...
    if args.output_format == "json":
        ...
        print(
            f"{result.run_a_name} (n={result.n_trajectories_a}) vs "
        ...
        for c in result.outcome_comparisons:
            ...
            print(
                f"  {c['category']}: {c['rate_a']:.1%} vs {c['rate_b']:.1%} "
        ...
        if result.metric_comparisons:
            ...
            for m in result.metric_comparisons:
                print(
                    f"  {m['metric']}: {m['mean_a']:.2f} vs {m['mean_b']:.2f} "
